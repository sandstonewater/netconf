...
[ -f snippets/editconfigs/DC348_test_config.txt ] && rm snippets/editconfigs/$filename_tun_pol_in_if_sum

## Create static route for Spine-Leaf layer
./create-static.sh $file_env_ios
...
do
  ...
  ## --python ncc.py --host $xr_mgmt -u $xr_user -p $xr_pass --do-edits tun-gre --params '{"TUN_NAME":"'$tunnel_name'","TUN_DESC":"'$TUNNEL_DESC'","TUN_SRC_IP":"'$gre_src_add'","TUN_SRC_INT":"'$bgp_us_int'","TUN_DEST":"'$i'"}'
  cat snippets/editconfigs/tun-gre.tmpl >> DC348_test_config.txt
  sed -i 's/{{TUN_NAME}}/'$tunnel_name'/g' DC348_test_config.txt
  sed -i 's/{{TUN_DESC}}/'$TUNNEL_DESC'/g' DC348_test_config.txt
  sed -i 's/{{TUN_SRC_IP}}/'$gre_src_add'/g' DC348_test_config.txt
  sed -i 's/{{TUN_SRC_INT}}/'$bgp_us_int'/g' DC348_test_config.txt
  sed -i 's/{{TUN_DEST}}/'$i'/g' DC348_test_config.txt

  ## --python ncc.py --host $xr_mgmt -u $xr_user -p $xr_pass --do-edits tun-static --params '{"DUMMY_PREFIX":"'$dummy_static_net'","TUN_NAME":"'$tunnel_name'"}'
  
  ## --python ncc.py --host $xr_mgmt -u $xr_user -p $xr_pass --do-edits tun-mpls --params '{"TUN_NAME":"'$tunnel_name'","MPLS_LABEL":"'$mpls_label'", "DUMMY_PREFIX":"'$dummy_static_net'"}'

  ##Check operational status of GRE tunnel##
  ## --python ncc.py --host $xr_mgmt -u $xr_user -p $xr_pass --get-oper --named-filter intf-brief --params '{"INTF_NAME":"'$tunnel_name'"}' > tun-state
